        ...
        <section class="w-4/5 mx-auto bg-white" data-aos="fade-up" data-aos-duration="3000">
            ...
            <div class="flex py-4 gap-6 justify-start max-xl:flex-col">
                <div
                    ...
                    @for ($i = 1; $i <= 5; $i++)
                        <button
                            class="room-btn w-[150px] max-sm:w-[140px] bg-white hover:bg-[#BCEB77] hover:text-[#37580C] hover:scale-110 text-[#32421E] font-bold py-3 px-4 border-2 border-[#88bb3d] rounded-lg text-lg cursor-pointer {{ $i == 1 ? 'active' : '' }}"
                            data-room="{{ $i }}">
                            ห้อง {{ $i }}
                        </button>
                    @endfor
                ...
                {{-- ข้อมูล --}}
                @for ($i = 1; $i <= 5; $i++)
                    <div class="room-data flex flex-col bg-[#F7FAF3] rounded-lg p-6 w-full {{ $i == 1 ? '' : 'hidden' }}"
                        data-room="{{ $i }}">
