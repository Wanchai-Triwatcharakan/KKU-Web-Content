        ...
        <div class="w-[60%] mx-auto flex flex-col justify-center gap-12 mt-20 max-sm:mt-12 items-center">
            ...
            @foreach ($regisPost as $post)
                <a href="{{ url('register/detail/' . $post->id) }}" target="_blank"
                    class=" z-50 flex justify-between bg-[#F7FAF3] hover:bg-[#84B750] group hover:shadow-[#84B750] rounded-2xl shadow-md max-sm:flex-col "
                    data-aos="fade-up" data-aos-anchor-placement="center-bottom" data-aos-duration="3000">
                    <div
                        class="flex flex-col gap-4 p-6 items-start justify-center w-[70%] max-sm:w-full max-sm:order-2  overflow-y-auto">
                        <p class="text-[#23404A] group-hover:text-white font-bold text-start text-2xl max-md:text-xl ">
                            {{ $post->title }}</p>
                        <div class="max-h-[200px] max-sm:h-[150px] overflow-y-auto">
                            <p
                                class="text-[#686868] group-hover:text-white font-[300] text-start text-lg max-sm:text-[1rem]">
                                {{ $post->description }}</p>
                        </div>
                    </div>
