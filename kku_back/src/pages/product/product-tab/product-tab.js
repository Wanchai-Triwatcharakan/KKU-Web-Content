...
import ModalAddProduct from "../product-modal/product-add-modal";
import ModalEditProduct from "../product-modal/product-edit-modal";
...
import { svDeleteProductByToken } from "../../../services/product.service";
...
const ProductTab = (props) => {
  ...
  useEffect(() => { 
    if(category.length > 0) {
      setProductFilterData().then(res => {
        setTotalData(res.length)
        setFilteredData(res.slice(limited.begin, limited.end))
      })
    }
  }, [productTab, productData, page, rowsPerPage, pageControl, category]);

  const setProductFilterData = async () => {
    const filtered = await productData.filter((f) => { 
      /* กรอง Tab */
      if(productTab === "1" && f.status_display === 0){
        return false;
      } else if(productTab === "2" && f.pin === 0){
        return false;
      } else if(productTab === "3" && f.status_display === 1){
        return false
      }
      /* กรอง Page */
      if(pageControl !== "") {
  ...
  const deleteHandler = (item) => {
    ...
    modalSwal.fire({
      ...
      if(result.isConfirmed){
        svDeleteProductByToken(item.id,item.language).then(res => {
    ...
    <Box className="product-tab-section" sx={{ width: '100%' }}>
      <TabContext value={productTab}>
          ...
          <TabList onChange={handleTabChange} aria-label="lab API tabs example">
              {tabLists.map((tab) => (
                <Tab className="product-tab-header" 
                  value={tab.value} 
                  key={tab.value} 
                  icon={tab.icon} 
                  label={t(tab.title)} />
          ...
          {tabLists.map((tab) =>  (
            <TabPanel className={`product-tab-body ${(isRowDisplay)?"asRow":"asColumn"}`} value={tab.value} key={tab.value}>
