...
import { svCreateDesign } from "../../../services/design.service";
...
/* Export Component */
const ModalAddDesign = (props) => {
  const { t } = useTranslation("design-page");
  const { isOpen, menuList, designsList, category, totalData } = props;
  ...
  const [ckValueFront, setCkValueFront] = useState("");
  const [ckValueDetail, setCkValueDetail] = useState("");
  const [ckValuePromotion, setCkValuePromotion] = useState("");
  ...
  const saveModalHandler = () => {
    ...
    formData.append("content_front", ckValueFront);
    formData.append("content_detail", ckValueDetail);
    formData.append("content_promotion", ckValuePromotion);
    ...
    /* Fetch Data */
    svCreateDesign(formData).then((res) => {
      setIsFetching(false);
      if (res.status) {
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterMoment}>
      <Modal
        ...
        className={"modal-add-design"}
        aria-labelledby="modal-add-design"
        aria-describedby="modal-add-design"
      >
              ...
              <div style={{ width: "100%" }} className="form-details">
                ...
                {addData.pin && (
                  <div style={{ marginTop: "1rem" }} className="ck-content">
                    <label className="ck-add-design">
                      รายละเอียดของบ้านบนหน้าเว็บ
                    </label>
                    <CKeditorComponent
                      ckNameId="ck-add-design"
                      value={ckValueFront}
                      onUpdate={setCkValueFront}
                    />
                  </div>
                )}
                <div style={{ marginTop: "1rem" }} className="ck-content">
                  <label className="ck-add-design">รายละเอียดของบ้าน</label>
                  <CKeditorComponent
                    ckNameId="ck-add-design"
                    value={ckValueDetail}
                    onUpdate={setCkValueDetail}
                  />
                </div>
                <div style={{ marginTop: "1rem" }} className="ck-content">
                  <label className="ck-add-design">โปรโมชัน</label>
                  <CKeditorComponent
                    ckNameId="ck-add-design"
                    value={ckValuePromotion}
                    onUpdate={setCkValuePromotion}
                  />
                </div>
                {/* <TextField
                  onChange={(e) =>
