...
import { svUpdateService } from "../../../services/service.service";
...
import "./service-edit-modal.scss";
import Box from "@mui/material/Box";
import Modal from "@mui/material/Modal";
import IconButton from "@mui/material/IconButton";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faAdd, faEdit, faMinus, faXmark } from "@fortawesome/free-solid-svg-icons";
import { FormControl, FormControlLabel, FormGroup, FormLabel, Radio, RadioGroup, Switch, TextField } from "@mui/material";
...
const ModalEditService = (props) => {
  ...
  const { isEdit,isOpen, menuList, servicesList, category, items } = props
  ...
  const saveModalHandler = (e) => {
    ...
    svUpdateService(editData.id, formData).then(res => {
      setIsFetching(false)
      if(res.status) {
        props.setClose({
          isEdit: false,
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterMoment}>
      <Modal
        ...
        className={"modal-edit-service"}
        aria-labelledby="modal-edit-service"
        aria-describedby="modal-edit-service" >
        <Box className="modal-custom">
          <div className="modal-header">
            <h2>
              <FontAwesomeIcon icon={faEdit} />
              <span>{t("แก้ไขบริการ")}</span>
            </h2>
            <IconButton
          ...
          <div className="modal-body overflow-scroll-custom">
            <fieldset className="modal-fieldset">
              <legend className="modal-legend">{t("ModalEditServiceTitle")}</legend>
              <div className={`box-radio-container cate-menu-list`}>
                <div className="brc-body overflow-scroll-custom">
                <FormControl className="brc-form-control">
                  <FormLabel id="box-radio">ประเภทบริการ</FormLabel>
                  <RadioGroup
                    aria-labelledby="box-radio"
                    name="controlled-radio-buttons-group"
                    value={radioResult}
                    onChange={(e) => setRadioResult(parseInt(e.target.value))}
                  >
              ...
              <div className="form-details">
                ...
                <FieldsetUI className="more-image-setting" label={t("รูปภาพเพิ่มเติม (ขนาดไม่เกิน 1300x1300 pixels)")}>
                ...
                <h3 className="service-detail-title">{t("รายละเอียด")}</h3>
                <TextField
                ...
                <div style={{marginTop: "1rem"}} className="ck-content">
                  {/* <label className="ck-edit-service">Content</label> */}
                  {ckValue !== null && (
                    <CKeditorComponent
                      ckNameId="ck-edit-service"
                      value={ckValue} 
                      onUpdate={setCkValue} 
                    />
                  )}
                ...
                <h3 className="service-detail-title">{t("การแสดงผล")}</h3>
