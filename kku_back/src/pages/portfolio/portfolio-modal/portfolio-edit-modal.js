...
import { svUpdatePortfolio } from "../../../services/portfolio.service";
...
import "./portfolio-edit-modal.scss";
...
const ModalEditPortfolio = (props) => {
  const { t } = useTranslation("portfolio-page");
  const { isEdit, isOpen, menuList, portfoliosList, category, items } = props;
  ...
  const saveModalHandler = (e) => {
    ...
    svUpdatePortfolio(editData.id, formData).then((res) => {
      setIsFetching(false);
      if (res.status) {
        props.setClose({
          isEdit: false,
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterMoment}>
      <Modal
        ...
        className={"modal-edit-portfolio"}
        aria-labelledby="modal-edit-portfolio"
        aria-describedby="modal-edit-portfolio"
      >
          ...
          <div className="modal-body overflow-scroll-custom">
            <fieldset className="modal-fieldset">
              <legend className="modal-legend">
                {t("ModalEditPortfolioTitle")}
              </legend>

              <div style={{ width: "100%" }} className="form-details">
                <FieldsetUI className="image-setting" label={t("ข้อมูลรูปภาพ (ขนาดไม่เกิน 1300x1300 pixels)")}>
                  <PreviewImageUI
                    setCurImg={setCurImg}
                ...
                {/* <div style={{ marginTop: "1rem" }} className="ck-content">
                  <label className="ck-edit-portfolio">Content</label>
                  {ckValue !== null && (
                    <CKeditorComponent
                      ckNameId="ck-edit-portfolio"
                      value={ckValue}
                      onUpdate={setCkValue}
                    />
                  )}
