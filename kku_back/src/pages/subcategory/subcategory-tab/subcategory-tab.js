...
import { svDeleteSubCategoryByToken } from "../../../services/category.service";
...
const tabLists = [ 
  ...
  { value: "1", title: "แสดงบนแถบเมนู (Header)", icon: <FontAwesomeIcon icon={faSitemap} /> },
  { value: "2", title: "แสดงบนแถบเมนู (Footer)", icon: <FontAwesomeIcon icon={faSitemap} /> },
  { value: "3", title: "แสดงบนเว็บไซต์", icon: <FontAwesomeIcon icon={faWindowRestore} /> },
  { value: "4", title: "ซ่อนบนเว็บไซต์", icon: <FontAwesomeIcon icon={faEyeSlash} /> },
]

const SubCategoryTab = (props) => {
  const { tabSelect, categoryData, subCategoryData, isRowDisplay, totalData, selectedCateName, selectedCateId } = props;
  ...
  }, [tabSelect, categoryData, subCategoryData, page, rowsPerPage]);
  ...
  const deleteHandler = (item) => {
    modalSwal.fire({
      ...
      if(result.isConfirmed){
        svDeleteSubCategoryByToken(item.id,item.language).then(res => {
                ...
                {filteredData.map((item,index) => (
                  <ContentCardUI 
                    ...
                    data={{
                      ...
                      image: item.cate_thumbnail_link,
