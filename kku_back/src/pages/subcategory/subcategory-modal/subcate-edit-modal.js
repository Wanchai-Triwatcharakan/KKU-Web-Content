...
import { svUpdateSubCategory } from "../../../services/category.service";
import RadioBoxUI from "../../../components/ui/radio-box/radio-box";

import "./subcategory-add-modal.scss";
import Box from "@mui/material/Box";
import Modal from "@mui/material/Modal";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faAdd, faEdit, faMinus, faTimes, faXmark } from "@fortawesome/free-solid-svg-icons";
...
const ModalEditCategory = (props) => {
  const { isOpen, menuList, categoryData, subCategoryData } = props
  ...
  useEffect(() => {
    if (appEditData !== null ) {
      ...
      for (let key in appEditData) {
        if(key === "is_main_page" || key === "is_bottomside" || key === "is_menu"){ 
      ...
      setCateId(newData.main_cate_id)
      setEditData({...newData, imageName: ""});
      if(newData.cate_thumbnail_link !== "") {
        setPreviews({file: null, src: uploadPath + newData.cate_thumbnail_link})
      }
    }
  }, [appEditData])
  ...
  const saveModalHandler = () => {
    ...
    if(previews.file) {
      ...
      const curImg = editData.cate_thumbnail_link.split("/");
      formData.append("ImageName", curImg[curImg.length - 1]);
    }
   
    let c = categoryData.filter(d => parseInt(d.id) === parseInt(cateId))
    formData.append('id', editData.id)
    formData.append('main_cate_id', cateId)
    ...
    formData.append('is_bottomside', (editData.is_bottomside)?1:0)
    formData.append('is_main_page', (editData.is_main_page)?1:0)
    formData.append('language', language)
    
    svUpdateSubCategory(editData.id, formData).then(res => {
      setIsFetching(false)
      if(res.status) {
  ...
  function removeImage() {
    setPreviews({ file: "", src: "" });
    setEditData({ ...editData, cate_thumbnail_link: "" });
  }
      ...
      <Box className="modal-custom">
        ...
        <div className="modal-body overflow-scroll-custom">
          <fieldset className="modal-fieldset">
            <legend className="modal-legend">{t("แก้ไขหมวดหมู่ย่อย")}</legend>
            ...
            <div className="form-details">
              ...
              <h3 className="category-detail-title">{t("การแสดงผล")}</h3>
              <div className="setting-controls">
                <div className="switch-form">
                  <FormGroup>
                    <FormControlLabel  control={<Switch onChange={(e)=> setEditData({...editData, is_menu: e.target.checked})} checked={editData.is_menu} />} label="แสดงบนแถบเมนู (Header)"  labelPlacement="start" />
                  </FormGroup>
                </div>
                <div className="switch-form">
                  <FormGroup>
                    <FormControlLabel  control={<Switch onChange={(e)=> setEditData({...editData, is_bottomside: e.target.checked})} checked={editData.is_bottomside} />} label="แสดงบนแถบเมนู (Footer)"  labelPlacement="start" />
                  </FormGroup>
                </div>
                <div className="switch-form">
                  <FormGroup>
                    <FormControlLabel  control={<Switch onChange={(e)=> setEditData({...editData, is_main_page: e.target.checked})} checked={editData.is_main_page} />} label="แสดงบนเว็บไซต์"  labelPlacement="start" />
                  </FormGroup>
                </div>
                {/* <div className="input-group">
                  <div className="inp"> 
                    ...
                    <span className="title">
                      {t("ลำดับ")} {editData.cate_position}
                    </span>
